...
export default function LoginPage() {
  ...
  const { themeClass, isDarkMode, isMounted } = useTheme(); // Added isDarkMode here
  ...
  const handleGoogleCredential = async (response) => {
    ...
    } catch { toast.error("Google login failed. Please try again.", { position: "top-center" }); }
  };

  const handleGoogleLogin = () => {
    const btn = document.querySelector("#google-login-btn-hidden div[role=button]");
    if (btn) btn.click();
        ...
        <div className={"p-6 sm:p-7 rounded-2xl border shadow-xl " + themeClass("bg-[#1c1c1e] border-gray-800", "bg-white border-gray-200")}>
          ...
          <div className="text-center mb-4">
            ...
            <p className={`text-sm mt-1 ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>
              Sign in to continue with Collabi
            </p>
          </div>
          ...
          <div id="google-login-btn-hidden" style={{ position: "absolute", opacity: 0, pointerEvents: "none", left: "-9999px" }} />
